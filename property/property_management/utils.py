...
def get_billing_period_for_date(base_date, type):
    ...
    """
    period = frappe.get_all("Billing Period", ["name"], [["period_type", "=", type], ["start_date", "<=", base_date],
                                                         ["end_date", ">=", base_date]])
    if period:
...
def get_next_period(period_name, offset=1):
    ...
    fetched_periods = frappe.get_all("Billing Period", fields=["name"], filters=filt, order_by=order_by,
