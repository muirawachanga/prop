...
class UtilityItemMeasurement(Document):
    ...
    def get_previous_measurement(self):
        prev = frappe.db.sql("SELECT name FROM `tabUtility Item Measurement` WHERE "
                             "measurement_status != 'Cancelled' AND tenancy_contract = %(tc)s "
                             "AND utility_item = %(ui)s "
                             "AND NAME != %(me)s AND reading_date < %(rd)s "
                             "ORDER BY reading_date DESC LIMIT 0, 1", {'tc': self.tenancy_contract,
    ...
    def validate_dates(self):
        if self.measurement_status == 'New' and not self.is_opening_entry:
            count = frappe.db.sql("SELECT count(*) FROM `tabUtility Item Measurement` WHERE "
                                  "measurement_status != 'Cancelled' AND tenancy_contract = %(tc)s "
                                  "AND utility_item = %(ui)s "
                                  "AND NAME != %(me)s AND reading_date >= %(rd)s ", {'tc': self.tenancy_contract,
                                                                                     'ui': self.utility_item,
                                                                                     'me': self.name,
                                                                                     'rd': self.reading_date
                                                                                     })
    ...
    def check_if_older_exists(self):
        if self.measurement_status == 'New' and not self.is_opening_entry:
            count = frappe.db.sql("SELECT count(*) FROM `tabUtility Item Measurement` WHERE "
                                  ...
                                  "AND NAME != %(me)s", {'tc': self.tenancy_contract,
                                                         'ui': self.utility_item,
                                                         'me': self.name
                                                         })
            if count[0][0]:
